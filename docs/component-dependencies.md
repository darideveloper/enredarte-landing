...
- **Scroll reveals (`gsap-scroll-reveals`)**: every relevant below-fold section now carries an Approach A reveal (`gsap.fromTo` `y + autoAlpha` + `clearProps`, no CSS pre-hiding — SSR HTML fully indexable) in its host Astro `<script>` (never inside React islands), with section-scoped `js-<prefix>-*` hooks, `gsap.matchMedia()` reduced-motion no-op branches, `mm?.revert()` on `astro:after-swap` + re-init on `astro:page-load`, and `transition:animate="none"` on animated roots. Home/`obras` grids use per-card triggers; small grids/rows use single trigger + stagger; immersive rows animate inner cells only (sticky wrappers untouched); heroes and `[fetchpriority=high]` LCP images never animate opacity; `BlogIndex` CSS `blog-enter` keyframes deleted in favor of GSAP; `PostCard` gained a `class` prop (hook passthrough); `global.css` has a `@media (prefers-reduced-motion: reduce)` guard for non-GSAP motion. Conversion islands (`BuyWidget`, `OrderFlow`, `DeliveryForm`, `ArtworkSlider` internals) and the viewer scrub timeline are reveal-free by rule.
