...
### Requirement: Price matches the active language
The `ImageRowCard` molecule SHALL derive the displayed price from the raw `priceMxn` and `priceUsd` numbers and the current page language: Spanish pages render the price in MXN and English pages render it in USD, using `Intl.NumberFormat` via the shared `formatPrice` helper. There is no separate currency toggle.

#### Scenario: Spanish page shows MXN
- **WHEN** `ImageRowCard` is rendered on a Spanish page (`lang === "es"`)
- **THEN** the price line is formatted in MXN (`MX$` symbol, Spanish-locale grouping)
...
#### Scenario: Desktop row picks half-width variant
- **WHEN** rendered on 1280px desktop
- **THEN** the browser selects the ~640-1080w variant, not the full original
