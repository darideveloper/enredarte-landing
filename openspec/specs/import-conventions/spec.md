# import-conventions Specification

## Purpose
Defines the standardized import rule enforced across all source files: cross-directory imports use `@/` path aliases, same-directory imports use `./` relative paths, external packages remain untouched, and an automated check prevents regression.
