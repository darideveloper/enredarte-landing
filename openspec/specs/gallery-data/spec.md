...
## Purpose
Provides the build-time data orchestration that sources gallery, curator, artist, artwork, and taxonomy data from the backend DRF API (`/api/artworks/…`), acts as the single source of truth for both the homepage gallery section and the gallery detail pages, and exposes the API-faithful types imported from the `api-client` capability.
