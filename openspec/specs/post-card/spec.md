...
## Purpose
Salon letterbox card for `PostSummary` entries in the blog grid, distinct from gallery `ImageCard` chrome, using paper/ink/crimson/muted/card-dark/border-theme tokens. Surfaces title, description, author+date via `pickPostField` and `Intl.DateTimeFormat`.
