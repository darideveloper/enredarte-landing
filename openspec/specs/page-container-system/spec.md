# page-container-system Specification

## Purpose
Single source of truth for horizontal page spacing: a two-tier container contract (canvas vs reading vs narrow) encoded as Tailwind v4 `@utility` variants (`container-site-canvas`, `container-site-reading`, `container-site-narrow`) so every page aligns with the landing edges and no hand-rolled page containers drift.
## Requirements
