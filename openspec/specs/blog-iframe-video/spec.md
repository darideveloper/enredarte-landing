# blog-iframe-video Specification

## Purpose
Iframe-only video embeds inside blog post content: a paragraph in `Post.content_es` / `content_en` that is exactly one YouTube or Vimeo URL renders as a responsive hardened lazy `<iframe>` player. No backend schema change, no `<video>` file support, no oEmbed fetching.
