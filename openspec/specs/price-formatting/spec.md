## Purpose
Defines how artwork prices are derived, formatted, and rendered across the landing based on the current page language. Establishes the `formatPrice` helper, the lang→currency rule, and the contract for carrying raw numbers through view models.
