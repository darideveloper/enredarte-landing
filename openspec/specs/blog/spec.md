...
## Purpose
SSG blog index (paginated grid, `page_size=11`) and per-post detail pages in both languages, rendering every `Post`/`PostSummary` field with Markdown via `marked` and localized SEO, built via Fork A isolated fetch. Polished Salon editorial layer (mosaic, featured, drop-cap, figure/table/code prose, deduplication, i18n share). Banner images are absolute URLs used verbatim.
## Requirements
